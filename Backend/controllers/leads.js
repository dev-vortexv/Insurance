...
const getList = async (req, res) => {
  ...
  query.deleted = false;
  let allLead = await Lead.find(query)
    .populate({
      path: "createdBy",
  ...
  const leadAllData = allLead.filter((item) => item.createdBy !== null);

  let totalRecords = leadAllData.length;

  res.send({ leadAllData, count: totalRecords });
};

const getTotalLead = async (req, res) => {
  const query = req.query;
  query.deleted = false;
  let allLead = await Lead.find(query).exec();

  let totalRecords = allLead.length;

  res.send({ allLead, count: totalRecords });
};
...
const getAllConversation = async (req, res) => {
  try {
    ...
    const conversationData = await Lead.aggregate([
      {
        $match: {
          deleted: false,
...
const view = async (req, res) => {
  try {
    ...
    let lead = leads[0];
    let populatedLead = await Lead.populate(lead, {
      path: "assigned_agent",
...
const deleteData = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Lead and related data deleted successfully." });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error." });
  }
...
const deleteMany = async (req, res) => {
  try {
    ...
    // console.log("deletedLeads", deletedLeads);
    ...
    res
      .status(200)
      .json({ message: "Leads and related data deleted successfully." });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error." });
  }
