...
const getList = async (req, res) => {
  ...
  const allEmailData = allData.filter((item) => item.createdBy !== null);

  let totalRecords = allEmailData.length;
  res.send({ allEmailData, count: totalRecords });
};
...
const deleteMany = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Emails deleted successfully", deleteManyEmails });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error deleting Emails ", error: err.message });
  }
};

export default { getList, add, view, deleteData, deleteMany };
