...
const getList = async (req, res) => {
  ...
  let allData = await Calls.find(query).populate("createdBy", [
    "firstName",
    "lastName",
  ]);
  ...
  res.send({ allCallData: allData, count: totalRecords });
};
...
const deleteData = async (req, res) => {
  try {
    let calls = await Calls.findByIdAndUpdate(
      { _id: req.params.id },
      { deleted: true }
    );
...
const deleteMany = async (req, res) => {
  try {
    ...
    // Delete the calls
    const deleteManyCalls = await Calls.updateMany(
      { _id: { $in: callIdsToDelete } },
      { deleted: true }
    );
    ...
    res
      .status(200)
      .json({ message: "Call deleted successfully.", deleteManyCalls });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error deleting Calls(s) ", error: err.message });
  }
};
