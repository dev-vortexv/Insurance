...
const getList = async (req, res) => {
  ...
  query.deleted = false;
  let allData = await emailTemplate
    .find(query)
    .populate({
      path: "createdBy",
  ...
  res.send({ allTemplatesData: result, count: totalRecords });
};
...
const edit = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ result, message: "Email Template updated successfully" });
  } catch (err) {
...
const deleteData = async (req, res) => {
  try {
    let emailtemplate = await emailTemplate.findByIdAndUpdate(
      { _id: req.params.id },
      { deleted: true }
    );
    res
      .status(200)
      .json({ message: "Email Template deleted successfully", emailtemplate });
  } catch (err) {
...
const deleteMany = async (req, res) => {
  try {
    ...
    const deleteManyemailtemplate = await emailTemplate.updateMany(
      { _id: { $in: emailtemplateIdsToDelete } },
      { deleted: true }
    );
    ...
    res.status(200).json({
      ...
      deleteManyemailtemplate,
    });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error deleting Email Template.", error: err.message });
  }
};

export default { getList, add, view, edit, deleteData, deleteMany };
