...
const getList = async (req, res) => {
  ...
  let allData = await Tasks.find(query).populate("createdBy", [
    "firstName",
    "lastName",
  ]);
  ...
  res.send({ taskAllData: allData, count: totalRecords });
};
...
const view = async (req, res) => {
  let tasks = await Tasks.findOne({ _id: req.params.id }).populate(
    "createdBy",
    ["firstName", "lastName"]
  );
  // .populate("lead_id", ["firstName", "lastName"])
...
const deleteData = async (req, res) => {
  try {
    let tasks = await Tasks.findByIdAndUpdate(
      { _id: req.params.id },
      { deleted: true }
    );
...
const deleteMany = async (req, res) => {
  try {
    ...
    // Delete the tasks
    const deleteManyTasks = await Tasks.updateMany(
    ...
    res
      .status(200)
      .json({ message: "Tasks deleted successfully.", deleteManyTasks });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error deleting Tasks(s).", error: err.message });
  }
};
