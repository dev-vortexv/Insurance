...
const getList = async (req, res) => {
  ...
  let allData = await Meetings.find(query).populate("createdBy", [
    "firstName",
    "lastName",
  ]);
  ...
  res.send({ allMeetingData: allData, count: totalRecords });
};
...
const view = async (req, res) => {
  let meetings = await Meetings.findOne({ _id: req.params.id }).populate(
    "createdBy",
    ["firstName", "lastName"]
  );
  // .populate("lead_id", ["firstName", "lastName"])
