...
const getList = async (req, res) => {
  ...
  let allContactData = await Contact.find(query)
    .populate({
      path: "createdBy",
  ...
  const AllContactData = allContactData.filter(
    (item) => item.createdBy !== null
  );

  let totalRecords = AllContactData.length;
  res.send({ allContactData, count: totalRecords });
};

const getTotalContact = async (req, res) => {
  const query = req.query;
  query.deleted = false;

  let allContactData = await Contact.find(query).exec();
...
const deleteData = async (req, res) => {
  try {
    ...
    const deletedcontact = await Contact.findByIdAndUpdate(contactId, {
      deleted: true,
    });
    ...
    res
      .status(200)
      .json({ message: "Contact and related data deleted successfully." });
  } catch (error) {
    console.log(error);
...
const deleteMany = async (req, res) => {
  try {
    ...
    const deletedLeads = await Contact.updateMany(
      { _id: { $in: contactIds } },
      { $set: { deleted: true } }
    );
    ...
    res
      .status(200)
      .json({ message: "Contacts and related data deleted successfully." });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error." });
  }
