...
const PaymentsDetails = () => {
  ...
  const getAllPaymentsDetails = async () => {
    const response = await getApi(userRole === 'admin' ? `payments/totalPayment` : `payments/totalPayment/?createdBy=${user_id}`);
    ...
    <>
      <Container>
        <Stack direction="row" alignItems="center" mb={5} justifyContent={'space-between'}>
          <Typography variant="h4">Payments Lists</Typography>
        </Stack>
        <TableStyle>
          <Box width="100%">
